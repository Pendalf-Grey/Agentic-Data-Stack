...
Это demo-набор synthetic Elasticsearch logs для проверки цепочки:

```text
Elasticsearch -> elasticsearch-connector -> ClickHouse -> MCP -> LibreChat -> Grafana
```

В репозитории уже лежит статический fixture:
...
Он не генерируется заново при каждом запуске. Это намеренно: тесты и ответы модели должны быть воспроизводимыми.
...
2. Берет готовый static fixture из `elasticsearch-synthetic-lab/fixtures/`.
