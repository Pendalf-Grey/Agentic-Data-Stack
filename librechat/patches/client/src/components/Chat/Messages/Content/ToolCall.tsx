...
const ADS_PROGRESS_PREFIX = '__ADS_PROGRESS__=';

function getAdsProgressMessage(args: string | undefined): string | undefined {
  if (!args) {
    return undefined;
  }
  const markerIndex = args.lastIndexOf(ADS_PROGRESS_PREFIX);
  if (markerIndex === -1) {
    return undefined;
  }
  const payload = args.slice(markerIndex + ADS_PROGRESS_PREFIX.length).split('\n', 1)[0];
  try {
...
function stripAdsProgressMarkers(args: string | undefined): string | undefined {
  return args?.replace(/\n__ADS_PROGRESS__=[^\n]*\n/g, '').trim();
}
...
export default function ToolCall({
  ...
  const adsProgressMessage = useMemo(() => getAdsProgressMessage(args), [args]);
  const displayArgs = useMemo(() => stripAdsProgressMarkers(args), [args]);

  const hasInfo = useMemo(
    () => (displayArgs?.length ?? 0) > 0 || (output?.length ?? 0) > 0,
    [displayArgs, output],
  );
        ...
        <div className="overflow-hidden" ref={expandRef}>
          {hasInfo && (
            <div className="my-2 overflow-hidden rounded-lg border border-border-light bg-surface-secondary">
              <ToolCallInfo input={displayArgs ?? ''} output={output} attachments={attachments} />
            </div>
          )}
        </div>
      </div>
