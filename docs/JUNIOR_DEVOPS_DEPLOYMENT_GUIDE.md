...
| LLM Observability | llm-gateway, Langfuse | вызывает модель и пишет traces | latency, errors, token usage, traces |
...
**llm-gateway** — маленький OpenAI-compatible proxy.
...
В этом проекте `llm-gateway` делает две вещи:
...
- Во-вторых, `llm-gateway` отправляет trace каждого LLM-запроса в Langfuse.
...
| `vm-app` | Grafana, MCP, llm-gateway, LibreChat, MongoDB, Langfuse, Redis, MinIO | 4 vCPU | 10 GB | 80 GB | UI/API и LLM observability |
...
| `vm-app` | Airflow, Grafana, MCP, llm-gateway, LibreChat, MongoDB, Langfuse, Redis, MinIO | 5 vCPU | 10 GB | 100 GB |
...
| `app-1` | LibreChat, MCP, llm-gateway, Grafana | 4-8 vCPU | 16 GB | 100 GB SSD | UI/API |
...
| llm-gateway | `3344` | LibreChat/app network | OpenAI-compatible proxy |
...
- `llm-gateway` — маленький OpenAI-compatible proxy для LibreChat.
...
LLM_GATEWAY_BASE_URL=http://llm-gateway:3344/v1
...
### 7.10 Создать Минимальный LLM Gateway

```bash
cat > llm-gateway/package.json <<'EOF'
{
  "name": "llm-gateway",
  "version": "1.0.0",
  "type": "module",
...
cat > llm-gateway/Dockerfile <<'EOF'
FROM node:22-alpine
WORKDIR /app
COPY package*.json ./
RUN npm install
...
cat > llm-gateway/src/server.js <<'EOF'
...
`llm-gateway` нужен, чтобы все запросы к модели проходили через одну точку и попадали в Langfuse traces.
...
В `.env` укажите Ollama как upstream для `llm-gateway`:
...
`host.docker.internal` нужен на macOS, потому что LibreChat и `llm-gateway` работают внутри Docker, а Ollama обычно запущена на host-машине.
...
После любого изменения `.env` пересоздайте LibreChat и `llm-gateway`:

```bash
docker compose up -d --force-recreate llm-gateway librechat
```

`llm-gateway` перечитает `UPSTREAM_OPENAI_BASE_URL` и `UPSTREAM_OPENAI_API_KEY`.

`render-config.sh` внутри LibreChat перечитает `LIBRECHAT_MODELS` и соберет `/app/librechat.yaml` с новым списком моделей.

Проверить, что `llm-gateway` видит upstream models:
...
`LANGFUSE_INTERNAL_URL` — адрес внутри Docker network, куда `llm-gateway` отправляет traces.

`LANGFUSE_PUBLIC_KEY` и `LANGFUSE_SECRET_KEY` — API keys проекта Langfuse.

Их использует `llm-gateway`, чтобы писать traces.
...
В этой системе трассировка сделана на уровне `llm-gateway`.
...
Если `LANGFUSE_ENABLED=true`, `llm-gateway` отправляет trace после каждого non-streaming и streaming chat completion.

Если Langfuse временно недоступен, `llm-gateway` не ломает ответ пользователю.

Он пишет warning в logs и продолжает отдавать ответ LibreChat.

### 16.6 Ограничения Langfuse В Этом Проекте

Langfuse видит те LLM-запросы, которые проходят через `llm-gateway`.

Если какой-то сервис ходит в модель напрямую, минуя `llm-gateway`, Langfuse этот вызов не увидит.

В текущей реализации `llm-gateway` пишет в Langfuse:
...
Проверить, что в LibreChat выбран endpoint, который идет через `llm-gateway`.
